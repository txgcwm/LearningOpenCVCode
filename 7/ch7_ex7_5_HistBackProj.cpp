...
#define patchy 61
		...
		for (i = 0; i < 2; ++i) {
			...
			//
			hist[i] = cvCreateHist(2, hist_size, CV_HIST_ARRAY, ranges, 1);
			...
			//
			hist_img[i] = cvCreateImage(cvSize(h_bins * scale, s_bins * scale),
										8, 3);
			cvZero(hist_img[i]);
		...
		if ((patch)) {
			...
			cvCalcBackProjectPatch(planes[1], ftmp, cvSize(61, 61), hist[0],
								   type, 1.0);
			printf("ftmp count = %d\n", cvCountNonZero(ftmp));
