					...
					CvMat ** data, CvMat ** responses)
{
	...
	seq =
		cvCreateSeq(0, sizeof(*seq), (var_count + 1) * sizeof(float), storage);
						   ...
						   char *filename_to_save, char *filename_to_load)
{
	...
	if (filename_to_load) {
		...
		CvMat *new_data =
			cvCreateMat(ntrain_samples * class_count, var_count + 1, CV_32F);
		CvMat *new_responses =
			cvCreateMat(ntrain_samples * class_count, 1, CV_32S);
						 ...
						 char *filename_to_save, char *filename_to_load)
{
	...
	if (filename_to_load) {
		...
		for (i = 0; i < ntrain_samples; i++) {
			...
			float *bit_vec =
				(float *)(new_responses->data.ptr + i * new_responses->step);
			for (j = 0; j < class_count; j++)
				bit_vec[j] = 0.f;
		...
		CvMat layer_sizes =
			cvMat(1, (int)(sizeof(layer_sz) / sizeof(layer_sz[0])), CV_32S,
				  layer_sz);
		mlp.create(&layer_sizes);
		printf("Training the classifier (may take a few minutes)...");
		mlp.train(&train_data, new_responses, 0, 0,
				  CvANN_MLP_TrainParams(cvTermCriteria
										(CV_TERMCRIT_ITER, 300, 0.01),
	...
	for (i = 0; i < nsamples_all; i++) {
		...
		int r =
			fabs((double)best_class - responses->data.fl[i]) <
			FLT_EPSILON ? 1 : 0;
...
int main(int argc, char *argv[])
{
	...
	if (i < argc ||
		(method == 0 ?
		 build_rtrees_classifier(data_filename, filename_to_save,
								 filename_to_load) : method ==
		 1 ? build_boost_classifier(data_filename, filename_to_save,
									filename_to_load) : method ==
		 2 ? build_mlp_classifier(data_filename, filename_to_save,
								  filename_to_load) : -1) < 0) {
